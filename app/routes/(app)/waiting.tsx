...
export const meta: MetaFunction = () => {
  return [
    { title: "Oczekujące" },
    {
      name: "description",
      content:
        "Najświeższe memy, gify, filmy i posty. Zanurz się w oceanie ciekawych i zabawnych treści.",
    },
  ];
};
const PAGE_SIZE = 10;
...
export default function App() {
  ...
  const swr = useSWRInfinite<{
    ...
    (index) =>
      `/api/fetch/feed?feature=waiting&page=${index + 1}${searchParams.has("q") ? `&search=${searchParams.get("q")}` : ""}`,
    {
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-3">
      {/* <div className="mx-3 my-3 flex items-center gap-x-3 rounded-lg border-[3px] border-b-0 border-t-0 border-slate-900 bg-slate-300/30 p-4">
        ...
        Posty oczekujące na zatwierdzenie
      </div> */}
