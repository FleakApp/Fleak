...
export const meta: MetaFunction = () => {
  return [
    { title: "Trendy" },
    {
      name: "description",
      content:
        "Zobacz najpopularniejsze treści na fleak. Rozkoszuj się śmiesznymi memami które uzyskują najwięcej interakcji.",
    },
  ];
};
...
export default function App() {
  ...
  const swr = useSWRInfinite<{
    ...
    (index) =>
      `/api/fetch/feed?feature=trending&page=${index + 1}${searchParams.has("q") ? `&search=${searchParams.get("q")}` : ""}`,
    {
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-3">
      {/* <div className="mx-3 my-3 flex items-center gap-x-3 rounded-lg border-[3px] border-b-0 border-t-0 border-slate-900 bg-slate-300/30 p-4">
        ...
        Posty sortowane według ilości komentarzy
      </div> */}
